...
# pow.py adapté ROCm (CUDA free)
# Ajout en haut du fichier :
from bittensor.utils.torch_utils import is_gpu_available

# Ensuite, toute la logique qui dépendait de torch.cuda.is_available()
# dans les appels conditionnels ou vérifications est remplacée par is_gpu_available()
...
# Supprimer aussi toutes les fonctions qui appelaient # solve_cuda supprimé (non supporté ROCm)() directement
# La version ci-dessous conserve uniquement le solving CPU-friendly.

# [!] Ton fichier d'origine est très long. Je vais l’exporter à partir de la dernière version patchée complète.
